    ...
    where TMessage : class
{
    ...
    private readonly ConcurrentDictionary<int, MessageId?> _liveTailStartMessageId = [];
        ...
        ILogger<PulsarTableView<TMessage>> logger)
    {
        ...
        ArgumentNullException.ThrowIfNull(projectorTopicReaderFactory);
        ...
        (_topic, _projectorTopicReaderFactory, _unitOfWorkFactory, _avroSerializer, _messageApplier, _storeMetadata, _logger) =
            (topic, projectorTopicReaderFactory, unitOfWorkFactory, avroSerializer, messageApplier, storeMetadata, logger);
    }
    ...
    public async Task<TopicBootstrapResult<TMessage>> StartBootstrapAsync(CancellationToken cancellationToken = default)
    {
        ...
        var readerTasks = topicHighWatermark.PartitionIds
                                            .DefaultIfEmpty(0)
                                            .OrderBy(partitionId => partitionId)
                                            .Select(partitionId => StartBootstrapByPartition(_topic,
                                                                                             partitionId,
        ...
        _logger.LogInformation("Starting bootstrap for topic {Topic} across {PartitionCount} partitions.", _topic, topicHighWatermark.PartitionIds.Count);
    ...
    private static TopicBootstrapResult<TMessage> GetResult(StoreRecoveryMode recoveryMode, List<Task<PartitionBootstrapResult<TMessage>>> readerTasks)
    {
        var partitionResults = readerTasks.Select(task => task.Result).ToArray();
        ...
        var deltaChanges = partitionResults
                            .OfType<PartitionRecoveredFromStateStore<TMessage>>()
    ...
    private async Task<TopicHighWatermark> GetHighWaterMarkForTopic(CancellationToken cancellationToken)
    {
        ...
        try
        {
            TopicHighWatermark topicHighWatermark = await _projectorTopicReaderFactory.CaptureHighWatermarkAsync(_topic, highWatermarkCts.Token);
    ...
    private async Task<MessageId> ResolveCheckpointStartMessageIdAsync(string topicName, int partitionId, CancellationToken cancellationToken)
    {
        using var uow = _unitOfWorkFactory.CreateBootstrap<TMessage>();
        ...
        return lastCheckpoint is not null
            ? ToDotPulsarMessageId(lastCheckpoint.LastProcessedMessageId, lastCheckpoint.TopicName)
            : throw new InvalidOperationException($"Checkpoint not found for topic {topicName} partition {partitionId} while recovery mode was already resolved as trustworthy.");
    }
    ...
    private async Task<StoreRecoveryMode> ResolveStoreRecoveryModeAsync(TopicHighWatermark topicHighWatermark, CancellationToken cancellationToken)
    {
        ...
        foreach (var partitionId in topicHighWatermark.PartitionIds.OrderBy(partitionId => partitionId))
        {
            TopicCheckpoint? checkpoint = await uow.CheckpointStorage.GetLastCheckpoint(_topic, partitionId, cancellationToken);

            if (checkpoint is null)
            {
                return new RebuildFromEarliest($"checkpoint_missing_partition_{partitionId}");
            }

            if (checkpoint.StoreId != _storeMetadata.StoreGenerationId)
            {
                return new RebuildFromEarliest($"checkpoint_store_generation_mismatch_partition_{partitionId}");
            }
        }
    ...
    private void SetLiveTailStart(int partitionId, MessageId highWatermarkPartitionedMessageId)
    {
        _liveTailStartMessageId.AddOrUpdate(partitionId, _ => highWatermarkPartitionedMessageId, (_, _) => highWatermarkPartitionedMessageId);
    }

    private async Task<PartitionBootstrapResult<TMessage>> StartBootstrapByPartition(
                                                        string topicName,
        int partitionId,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            RecoverFromStateStore => await ResolveCheckpointStartMessageIdAsync(topicName, partitionId, cancellationToken),
        ...
        try
        {
            ...
            if (!needsBootstrap)
            {
                SetLiveTailStart(partitionId, highWatermarkMessageId);
                return recoveryMode switch
                {
                    RecoverFromStateStore => new PartitionRecoveredFromStateStore<TMessage>(partitionId, []),
                    RebuildFromEarliest => new PartitionRebuiltFromEarliest<TMessage>(partitionId),
                    _ => throw new UnreachableException()
                };
            }
            ...
            await using var reader = await _projectorTopicReaderFactory.CreateReaderAsync(topicName,
                                                                                         partitionId,
                                                                                         startMessageId,
                                                                                         cancellationToken);
            ...
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                var currentId = ToDotPulsarMessageId(message.BrokerMessageId, topicName);
                if (currentId.CompareTo(highWatermarkMessageId) >= 0)
                {
                    _logger.LogInformation("Bootstrap finalizado para {Topic} en el mensaje {Id}", topicName, message.BrokerMessageId);
                    break;
                }
            ...
            _liveTailStartMessageId.AddOrUpdate(partitionId, _ => highWatermarkMessageId, (_, _) => highWatermarkMessageId);

            return recoveryMode switch
            {
                RecoverFromStateStore => new PartitionRecoveredFromStateStore<TMessage>(partitionId, changes),
                RebuildFromEarliest => new PartitionRebuiltFromEarliest<TMessage>(partitionId),
                _ => throw new UnreachableException()
            };
        }
